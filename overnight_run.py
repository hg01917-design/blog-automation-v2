...
def _naver_short_url(url: str) -> str:
    """네이버 QR코드 서비스로 m.site.naver.com 단축 URL 생성 (실패 시 원본 반환)"""
    import re as _re
    try:
        from browser import connect_cdp, get_or_create_page
        pw, browser = connect_cdp()
        try:
            # naver.com 탭 재사용 (없으면 새로 열기)
            page = get_or_create_page(browser, url_contains="naver.com")

            log("[단축URL] qr.naver.com/create 접속 중...")
            ...
            if "login" in page.url or "nid.naver.com" in page.url:
                log("[단축URL] 로그인 필요 — 원본 URL 반환")
                return url

            # URL 입력창 찾기 — 여러 selector 순차 시도
            url_input = None
            for sel in [
                "input[type='url']",
            ...
            if not url_input:
                ...
                if inputs:
                    ...
                    log("[단축URL] URL 입력창 없음 — 원본 URL 반환")
                    return url
            ...
            if btn:
                ...
                log("[단축URL] 생성 버튼 없음 — 원본 URL 반환")
                return url

            # 결과 URL 추출 — HTML 소스에서 패턴 매칭
            ...
            for pat in [
                ...
                if matches:
                    log(f"[단축URL] 생성 성공: {matches[0]}")
                    return matches[0]

            # input[readonly] 등 locator 기반 추출
            for sel in ["input[readonly]", ".short_url", ".result_url",
                        "[class*='short']", "[class*='result']", "[class*='url']"]:
                loc = page.locator(sel)
                if loc.count() > 0:
                    try:
                        val = loc.first.input_value() or loc.first.inner_text()
                        if val and ("naver" in val or "http" in val):
                            log(f"[단축URL] 생성 성공 ({sel}): {val.strip()}")
                            return val.strip()
                    except Exception:
                        pass

            log("[단축URL] 단축 URL 추출 실패 — 원본 반환")
        finally:
            pw.stop()
    except Exception as e:
...
def run_posting_pipeline(blog_id, keyword, _resume=None):
    ...
    # goodisak: blogspot_it 백링크 삽입 (링크 피라미드: WP → Blogspot → Tistory)
    if blog_id == "goodisak":
        ...
        if _bs_it_url:
            keyword_final += (
                ...
                f"\n"
                f"삽입 형식 (HTML 그대로 사용):\n"
                f"<a href=\"{_bs_it_url}\" target=\"_blank\" "
                f"style=\"display:inline-block;background:#1a73e8;color:#fff;"
                f"padding:11px 22px;border-radius:6px;text-decoration:none;"
                f"font-weight:bold;font-size:15px;letter-spacing:-0.3px;\">"
                f"→ {keyword} 관련 정보 더 보기</a>\n"
            ...
            log(f"[파이프라인] goodisak blogspot_it 백링크 주입: {_bs_it_url}")
...
def _blogger_insert_images_pw(blogger_blog_id: str, post_id: str, image_paths: dict, on_log=None) -> bool:
    """Blogger 에디터를 Playwright로 열어 이미지 삽입 후 저장.
    ...
    log(f"[Blogger이미지] 에디터 열기: {edit_url}")
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            # 에디터 iframe 또는 contenteditable 찾기
            editor = None
            try:
                editor = page.frame_locator("iframe.docs-texteventtarget-iframe, iframe#editor").first
            except Exception:
                pass

            # H2 헤딩 요소 찾기 (contenteditable 내부)
            h2_els = page.locator("h2, [data-text-type='heading2']").all()
            if not h2_els:
                # 에디터 body에서 직접 찾기
                h2_els = page.locator("div[contenteditable='true'] h2").all()
            log(f"[Blogger이미지] H2 감지: {len(h2_els)}개")

            sorted_imgs = sorted(image_paths.items())
            for idx, (img_num, img_path) in enumerate(sorted_imgs):
                ...
                # 이미지 삽입 버튼 클릭 (Blogger 툴바)
                img_btn = None
                for sel in [
                    'button[aria-label*="이미지"]',
                    'button[aria-label*="Image"]',
                    'button[data-tooltip*="이미지"]',
                    'div[aria-label*="이미지 삽입"]',
                ]:
                    try:
                ...
                if not img_btn:
                    log(f"[Blogger이미지] 이미지 버튼 못 찾음, 이미지{img_num} 스킵")
                    continue
                ...
                # 파일 업로드 탭 클릭
                for upload_sel in ['button:has-text("컴퓨터에서")', 'button:has-text("Upload")', 'label[for*="upload"]']:
                    try:
                ...
                # 확인 버튼
                for ok_sel in ['button:has-text("선택")', 'button:has-text("확인")', 'button:has-text("OK")', 'button:has-text("Add selected")']:
                    try:
                        ...
                        pass
                _t.sleep(1)
                log(f"[Blogger이미지] 이미지{img_num} 삽입 완료: {Path(img_path).name}")

            # 저장 (게시)
            for save_sel in ['button:has-text("게시")', 'button:has-text("Publish")', 'button:has-text("저장")', 'button:has-text("Save")']:
                try:
                    btn = page.locator(save_sel).first
                    if btn.is_visible(timeout=2000):
                        btn.click()
                        _t.sleep(2)
                        log("[Blogger이미지] 저장 완료")
                        break
                except Exception:
                    pass
