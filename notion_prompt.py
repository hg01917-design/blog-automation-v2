...
**Rank Math SEO 최적화 규칙 (baremi542) — 100점 목표**:
- 본문 첫 문장(50자 이내)에 메인 키워드 '{keyword}'를 **정확히 그대로(띄어쓰기·철자 변형 절대 금지)** 포함
  예시: '{keyword}를 찾고 계신 분들을 위해 직접 정리해봤어요.'
- 본문 전체에서 '{keyword}' 표현을 최소 5회 이상 그대로 사용 (키워드 밀도 1~3%)
- H2 소제목 3개 모두에 '{keyword}' 포함 (예: ## {keyword} 사용 방법 — 단계별 안내)
- 각 H2 앞에 id 앵커 포함: ## H2 제목 {{#section-N}} 형식 아니라 마크다운 그대로 작성
- 최소 600 단어 이상 작성 (한글 기준 약 1800자 이상)
- 내부 링크 1개 이상: 반드시 **특정 포스트 URL**로 연결 (홈페이지 URL만 넣으면 Rank Math 미인식). 예: [실업급여 신청 방법](https://baremi542.com/실업급여-신청-기간/)
- 외부 공식 링크 1개 이상: 해당 정부/공공기관 공식 URL (DoFollow)
- SEO 제목에 파워워드 포함: 핵심, 완벽, 필수, 쉬운, 빠른, 정확한 중 1개 이상
- SEO 제목에 감성어 포함: 쉽게, 간단히, 완벽하게, 빠르게, 정확히 중 1개 이상
- 예: '{keyword} 핵심 정리 | 쉽게 계산하는 방법 2024'

**Rank Math 100점 고정 구조 템플릿 (반드시 이 순서대로 작성)**:

1. **도입부** (키워드로 시작하는 1~2문장)
   - 예: "{keyword}를 찾고 계신 분들이 많아서 직접 확인하고 정리해봤어요."

2. **목차** (## 목차 — 아래 3개 섹션 링크)
   - 예:
     ## 목차
     - [{keyword} 사용 방법](#section-1)
     - [{keyword} 핵심 정보 표](#section-2)
     - [{keyword} 신청자격 확인](#section-3)

3. **내부 링크 + 도입 설명** (1~2문단)
   - [baremi542 정부지원금 블로그](https://baremi542.com) 자연스럽게 포함

4. **## {keyword} 사용 방법 — [부제목] (section-1)**
   {{이미지1}}
   - 구체적인 단계별 사용/신청 방법 (300자 이상)
   - '{keyword}' 2회 이상 사용

5. **## {keyword} [핵심 정보] — [부제목] (section-2)**
   [애드센스]
   - 마크다운 표 (| 컬럼 | 컬럼 | 형식, 5행 이상)
   - 표 아래 설명 2~3문단 ('{keyword}' 1회 이상)
   - 외부 공식 링크 포함

6. **## {keyword} [추가 정보/주의사항] (section-3)**
   [애드센스]
   - 상세 설명 3~4문단 ('{keyword}' 1회 이상)
   - 굵게(**) 강조 포인트 3개 이상

7. **마무리 CTA**
   - [신청하러 가기](공식URL)

위 7단계 구조를 **빠짐없이** 모두 포함해야 함. 섹션 누락 시 Rank Math 점수 하락."""
