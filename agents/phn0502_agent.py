"""phn0502 전용 에이전트 — 영화/OTT 드라마 블로그 (film.baremi542.com)"""
import re
import sys
...
BLOG_ID = "phn0502"
PERSONA_RULE = (
    "phn0502 영화/드라마 블로그 운영자 본인 시점 — 영화·OTT 1편 심층 분석 전문가. "
    "줄거리 요약 금지. 연출 방식·배우 연기 포인트·독특한 설정에 집중. "
    "마지막에 시청 가능한 OTT 명시 필수. 무속/종교/귀신 소재 영화 취급 금지. "
    "판단 기준: '독자가 오늘 저녁 이 작품을 보고 싶어지는가?'"
)
...
def run(keyword: str, on_log=None, on_status=None, skip_images=False, extra_context=None):
    ...
    if result["images"] and not skip_images:
        log(f"[작성] 이미지 {len(result['images'])}개 생성 시작 (phn0502: Bing)")
