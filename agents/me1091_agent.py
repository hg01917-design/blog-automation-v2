"""me1091 전용 에이전트 — 쿠팡파트너스 제품후기 네이버 블로그"""
import re
import sys
...
BLOG_ID = "me1091"
PERSONA_RULE = (
    "me1091 쿠팡파트너스 제품후기 블로그 운영자 본인 시점. "
    "SCQA+MPC 프레임워크 기반 구매 전환율 높은 리뷰 작성. "
    "장점만 나열 금지 — 단점도 솔직하게. 감각적 묘사+수치 필수. "
    "쿠팡파트너스 공정위 고지문 본문 상단 삽입 필수. "
    "OGP 링크 카드 형식으로 쿠팡 상품 링크 삽입. "
    "판단 기준: '이 리뷰를 읽고 구매 여부를 결정할 수 있는가?'"
)
...
def run(keyword: str, on_log=None, on_status=None, skip_images=False, extra_context=None):
    ...
    # me1091은 네이버 — Gemini 이미지만 사용 (loremflickr 절대 금지)
    image_paths = {}
    if result["images"] and not skip_images:
        log(f"[작성] 이미지 {len(result['images'])}개 생성 시작 (me1091: Gemini 전용)")
