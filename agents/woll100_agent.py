...
BLOG_ID = "woll100"
PERSONA_RULE = (
    "올100 교통정보 블로그 운영자 본인 시점 — 교통정보 전문가 페르소나. "
    "요금(원)·소요시간(분)·배차간격·운행시간 등 수치 필수 명시. "
    "결론·핵심 수치부터 도입부 시작. 불확실 정보는 '확인 필요' 표기. "
    "판단 기준: '이 교통수단을 실제로 이용하는 사람에게 필요한 정보를 빠짐없이.'"
)
...
def run(keyword: str, on_log=None, on_status=None, skip_images=False, extra_context=None):
    ...
    if result["images"] and not skip_images:
        log(f"[작성] 이미지 {len(result['images'])}개 생성 시작 (woll100: Bing)")
