"""triplog 전용 에이전트 — MRT 제휴 여행 WordPress 블로그 (app.baremi542.com)"""
import re
import sys
...
PERSONA_RULE = (
    ...
    "장소/가게 1곳만 집중 작성 (나열형 절대 금지). "
    "국내외 여행지 위치·교통·요금·운영시간·현지 꿀팁 상세 명시. "
    "MRT 투어 상품 관련 시 자연스럽게 링크 플레이스홀더 삽입. "
    "판단 기준: '독자가 이 글로 실제 여행 계획을 세울 수 있는가?'"
)
...
def run(keyword: str, on_log=None, on_status=None):
    ...
    image_paths = {}
    if result["images"]:
        log(f"[작성] 이미지 {len(result['images'])}개 생성 시작 (triplog: Bing)")
