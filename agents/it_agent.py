...
def run(keyword: str, on_log=None, on_status=None, skip_images=False):
    ...
    if result["images"] and not skip_images:
        log(f"[작성] 이미지 {len(result['images'])}개 생성 시작 (goodisak: Bing→Pollinations)")
        image_paths = _img_router("goodisak", result["images"], skip_webp=False, on_log=log, title=result.get("title", ""))
