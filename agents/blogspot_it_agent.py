"""blogspot_it 전용 에이전트 — tech.baremi542.com (Blogger API 발행)"""
import re
import sys
...
BLOG_ID = "blogspot_it"
PERSONA_RULE = (
    "tech.baremi542.com 한국어 IT/테크 블로그 운영자 본인 시점. "
    "구글 한국어 SEO 타겟. 장소/가게 1곳만 집중 작성 (나열형 절대 금지). "
    "마크다운 형식 필수 (## 소제목, **볼드**). 구분선(---, ***) 절대 금지. "
    "판단 기준: '독자가 이 글 하나로 제품/서비스를 바로 이해하고 사용할 수 있는가?'"
)
...
def run(keyword: str, on_log=None, on_status=None, skip_images=False):
    ...
    if result["images"] and not skip_images:
        log(f"[작성] 이미지 {len(result['images'])}개 생성 시작 (blogspot_it: Bing)")
