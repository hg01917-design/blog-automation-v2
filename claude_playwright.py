            ...
            if blog_id == "triplog":
                ...
                if is_itinerary:
                    prompt += (
                        ...
                        "1. 도입부: 이 여행을 계획하는 사람의 고민(비용? 일정? 혼자?) 공감으로 시작\n"
                        "2. 날짜별 동선 (구체적 시간·이동 방법·비용 포함)\n"
                        "3. 각 코스에서 마이리얼트립 투어/액티비티 자연스럽게 언급 (가격 포함)\n"
                        "   예) '여기서 현지 가이드 투어 신청하면 훨씬 편했는데, 1인 35,000원이에요'\n"
                        "4. 숙소·맛집 추천 + 실제 예산 총정리\n"
                        "5. 맺음말: 예약 전 체크리스트 or '이런 분께 추천' 문장으로 마무리\n"
                        "제휴 링크가 extra_context로 제공되면 반드시 본문 중간 자연스러운 위치 + 맺음말 직전에 삽입해.\n"
                    )
                else:
                    prompt += (
                        "\n\n[제휴 마케팅 장소 심층 작성 규칙 — 필수]\n"
                        "독자가 '이 투어 신청해야겠다'고 느끼도록 써야 해.\n"
                        "1. 도입부: 이 장소를 검색하는 사람의 기대/고민으로 공감 시작\n"
                        "2. 직접 다녀온 것처럼 구체적 묘사 (무엇을 봤는지, 어떤 느낌인지)\n"
                        "3. 혼자 가면 아쉬운 포인트 언급 → 마이리얼트립 가이드 투어 자연스럽게 소개\n"
                        "   예) '한자를 모르니까 설명이 없으면 그냥 지나칠 수밖에 없더라고요. 그래서 가이드 투어 신청했는데 완전 다른 경험이었어요'\n"
                        "4. 실용 정보: 입장료, 운영시간, 가는 법, 소요 시간\n"
                        "5. 제휴 링크가 extra_context로 제공되면 본문 중간 + 맺음말 앞에 반드시 삽입\n"
                        "   링크는 <a href='URL' target='_blank'>상품명 (가격)</a> 형식으로\n"
                    )
                ...
                prompt += (
                    ...
                    "1. 인트로: 이 상품을 왜 찾는지 소비자 니즈를 짚어주며 시작. '직접 구매해서 써봤는데 좋아서 소개한다'는 자연스러운 뉘앙스.\n"
                    "2. 가격 정보: 인트로 바로 아래에 아래 형식으로 삽입\n"
                    "   일반회원: ~원\n"
                    "   와우회원: ~원\n"
                    "   (가격 정보가 없으면 '쿠팡 최저가 확인하기'로 대체)\n"
                    "3. 사용후기(본문): 상품의 특징·장점·단점을 실사용 경험처럼 자연스럽게 서술. 800~1200자.\n"
                    "4. 맺음말: 어떤 사람에게 추천하는지 한두 줄로 마무리.\n"
                    "\n[작성자 설정 — 쿠팡 리뷰어]\n"
                    "점잖은 30대 여성의 톤앤매너. '~했습니다', '~좋더라구요', '~이던데 좋더라구요' 등 자연스럽게 섞어서 사용.\n"
                    "리뷰어가 직접 경험한 것처럼 쓰되, '이것만 사세요' 같은 억지 강요 표현 금지.\n"
                    "제목은 [상품명] [특징키워드] [쿠팡 가격/할인] [사용후기] 나열형으로 작성.\n"
