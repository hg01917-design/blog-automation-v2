...
def generate_text(prompt: str, blog_id: str = None, keyword: str = None,
    ...
    _BOLD_RULE = (
        ...
        "제목에 '완벽가이드', '완벽정리', '완벽정복', '완벽 가이드', '완벽 정리', '완벽 정복', '총정리', '마이리얼트립', '한눈에 보기', '한눈에 정리', '알아보기', '쉽게 알아보는', '대해서 알아보자' 등 마케팅/요약 문구 절대 금지.\n"
    ...
    _FORMAT_RULE = (
        "\n\n[제목 규칙 — 필수]\n"
        "===제목=== 안의 제목에 '완벽가이드', '완벽정리', '완벽정복', '완벽 가이드', '완벽 정리', '완벽 정복', '총정리', '마이리얼트립', '한눈에 보기', '한눈에 정리', '알아보기', '쉽게 알아보는', '대해서 알아보자' 절대 금지.\n"
        "제목이 '정리'로 끝나는 것도 금지 (예: '~항목 정리', '~내용 정리'). 단 '정리수납' 같은 합성어는 허용.\n"
    ...
    if blog_id and keyword:
        if blog_id in BLOG_PROJECT_URLS:
            ...
            if blog_id in {"nolja100", "triplog"}:
                ...
                prompt += (
                    "\n\n[주제 심층 작성 규칙 — 필수]\n"
                    "이 키워드를 검색하는 사람은 '어떤 혜택/지원이 있는지' 구체적으로 모르는 상태야.\n"
                    "키워드에서 가장 적합한 정부지원 제도·복지 혜택·IT 기능 1가지를 직접 선정해서 그것만 깊게 작성해.\n"
                    "예) '정부지원금 신청 대상자 2026' → '청년도약계좌' 1가지 집중\n"
                    "여러 제도를 나열하는 글 금지. 1가지의 대상조건·신청방법·금액·주의사항을 구체적으로 작성해.\n"
                )
