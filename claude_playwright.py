...
BLOG_PROJECT_URLS = {
    ...
    # issue01: 블로그 생성 후 Claude.ai 프로젝트 만들고 URL 추가
    # "issue01": "https://claude.ai/project/...",
}
...
def generate_text(prompt: str, blog_id: str = None, keyword: str = None,
    ...
    _BOLD_RULE = (
        ...
        "제목에 '완벽가이드', '완벽정리', '완벽정복', '완벽 가이드', '완벽 정리', '완벽 정복', '완전 정리', '완전정리', '총정리', '꿀팁', '마이리얼트립', '한눈에 보기', '한눈에 정리', '알아보기', '쉽게 알아보는', '대해서 알아보자' 등 마케팅/요약 문구 절대 금지.\n"
    ...
    _FORMAT_RULE = (
        ...
        "금지 단어: '압도적으로', '환상적이다', '환상적인', '마치', '놀라운', '실로', '탁월한', '눈에 띄게', '뛰어나다', '한마디로', '경이로운'\n"
        "금지 패턴: '~살펴보겠습니다', '~알아보겠습니다', '~라고 할 수 있습니다', '~라고 볼 수 있습니다', '다양한 ~', '여러 가지 ~', '~해 보겠습니다', '종합적으로', '결론적으로', '다양한 측면에서', '물론입니다', '당연히'\n"
        "금지 문장: '100% 확신합니다', '무조건 추천드려요', '완벽한 선택입니다'\n"
        "강조 남발 금지: '가장 중요해요', '정말 중요해요', '매우 중요합니다' — 같은 강조를 한 글에 2번 이상 쓰지 말 것\n"
        "설명조 어미 남발 금지: '충분히 ~에요', '~라고 할 수 있어요', '~인 셈이에요' 반복 금지\n"
        ...
        "본문 길이: 공백 제외 순수 텍스트 **반드시 순수 텍스트 3000자** 작성 (소제목 4~5개, 각 소제목 아래 3~5문단).\n"
        "\n[글쓰기 방식 — 필수]\n"
        "'TOP 3', '5가지 방법', 'N개의 팁' 식의 나열형 구성 절대 금지.\n"
        "하나의 주제를 깊이 있게 파고드는 글 구성 (예: 특정 장소 한 곳 집중, 특정 제품 하나 심층 리뷰).\n"
        ...
        if blog_id in BLOG_PROJECT_URLS:
            ...
            if blog_id == "triplog":
                ...
                prompt += (
                    ...
                    "\n[클릭 전환 핵심 규칙 — 필수]\n"
                    "제휴 링크는 본문 상단(첫 번째 소제목 아래)에 먼저 삽입하고, 맺음말 직전에도 반드시 한 번 더 삽입해.\n"
                    "두 번 노출 이유: 스크롤 끝까지 안 읽는 독자도 클릭 기회를 줘야 해.\n"
                    "링크 형식: <a href='URL' target='_blank' style='color:#1a73e8;font-weight:bold;'>상품명 예약하기 (1인 가격원부터)</a>\n"
                    "링크 바로 위에 1줄 후킹 문구 반드시 삽입:\n"
                    "  예) '성수기에 금방 마감되는 인기 투어예요 — 날짜 먼저 잡아두는 게 좋더라고요'\n"
                    "  예) '저는 이 투어로 예약했는데 한국어 가이드라서 설명이 훨씬 편했어요'\n"
                )
                ...
                if is_itinerary:
                    prompt += (
                        ...
                        "1. 도입부: 이 여행을 고민하는 사람의 구체적인 상황·고민 공감으로 시작 (비용? 혼자? 가족?)\n"
                        "   예) '항공+숙소는 잡아놨는데 현지에서 뭘 할지 몰라서 한참 찾아봤거든요'\n"
                        "2. 제휴 투어 조기 노출: 첫 소제목 아래에 '이 여행에서 제일 좋았던 투어' 먼저 소개 + 링크 삽입\n"
                        "   후킹: '미리 예약 안 하면 현지에서 당일 못 탈 수 있어요'\n"
                        "3. 날짜별 동선: 구체적 시간·이동 방법·비용 포함. 각 코스에서 MRT 투어 자연스럽게 언급\n"
                        "   예) '여기서 현지 가이드 투어 신청하면 훨씬 편했는데, 1인 35,000원이에요'\n"
                        "4. 예산 총정리: 항공·숙소·투어·식사 각각 실제 예산 명시 (구체적 숫자 필수)\n"
                        "5. 맺음말: '이런 분께 추천' + 제휴 링크 재삽입 + '지금 날짜 확인해보세요' CTA\n"
                    )
                else:
                    prompt += (
                        ...
                        "   예) '혼자 가면 어떻게 돌아봐야 할지 몰라서 검색하다가 결국 투어로 예약했어요'\n"
                        "2. 제휴 투어 조기 노출: 첫 소제목 아래 '이 장소는 이 투어로 예약했어요' + 링크 삽입\n"
                        "   후킹 문구: '혼자 오면 설명 없이 보고 끝인데 투어는 달랐어요'\n"
                        "3. 직접 다녀온 것처럼 구체적 묘사: 분위기, 볼거리, 시간대별 추천\n"
                        "4. '혼자 갔을 때 아쉬웠던 점' → 투어 신청 후 달라진 경험 대비해서 서술\n"
                        "5. 실용 정보: 입장료, 운영시간, 가는 법 (구체적 번호·금액)\n"
                        "6. 맺음말: '이런 분께 추천' + 제휴 링크 재삽입 + 희소성 문구\n"
                        "   예) '비수기에도 금방 마감되더라고요 — 먼저 날짜 잡아두세요'\n"
                    )
                ...
                prompt += (
                    f"\n\n⚠️ 반드시 '{keyword}' 주제만 작성할 것. 다른 주제 절대 금지.\n"
                    "\n[주제 심층 작성 규칙 — 필수]\n"
                    "이 키워드를 검색하는 사람은 '어떻게 하면 되는지' 구체적인 방법을 모르는 상태야.\n"
                    f"'{keyword}'에 관한 살림 방법·절약 팁·생활 정보를 구체적으로 깊게 작성해.\n"
                    "여러 방법을 나열하는 글 금지. 1가지를 단계별로 구체적으로 작성해.\n"
                )
            elif blog_id in {"me1091"}:
                prompt += (
                    "\n\n[쿠팡파트너스 리뷰 작성 규칙 — 필수]\n"
                    "me1091은 쿠팡 상품 리뷰 블로그야. 반드시 아래 순서로 작성해:\n"
                    "1. 인트로(후킹): 이 상품을 고민하는 사람의 상황·고민을 공감으로 시작. '저도 한참 고민했는데 결국 이걸로 결정했어요' 뉘앙스.\n"
                    "2. 가격 앵커링: 인트로 바로 아래에 아래 형식으로 삽입 (클릭 전환 핵심)\n"
                    "   💰 현재 쿠팡 가격\n"
                    "   일반회원: ~원 / 와우회원: ~원 (X% 할인)\n"
                    "   📦 로켓배송: 오늘 주문 시 내일 도착\n"
                    "   (가격 정보 없으면 '쿠팡에서 최저가 확인 →' 형식으로)\n"
                    "3. 왜 이걸 골랐나(구매 이유): 다른 제품 대신 이걸 선택한 이유를 구체적으로. '비슷한 가격대 중에서 이게 제일 낫더라고요' 뉘앙스.\n"
                    "4. 실사용 후기(본문): 구체적 사용 경험. 장점 위주, 아쉬운 점 1가지 솔직하게. 800~1200자.\n"
                    "   - 쿠팡 실 구매자 리뷰 트렌드 (예: '리뷰 3만 개 넘게 달렸는데', '평점 4.7이더라고요') 자연스럽게 언급\n"
                    "5. 이런 분께 추천: 구체적인 대상(예: 자취하는 분, 선물용, 가성비 찾는 분)\n"
                    "6. 맺음말 + 구매 유도: '지금 쿠팡에서 가격 확인해보세요 — 와우 회원이면 더 저렴하게 살 수 있어요' 형식으로 마무리.\n"
                    "   제휴 링크가 extra_context로 제공되면 가격 앵커링 섹션 + 맺음말 직전에 반드시 삽입.\n"
                    "\n[작성자 설정 — 쿠팡 리뷰어]\n"
                    "실제로 구매해서 써본 30대 생활 블로거 톤. '~했어요', '~좋더라고요', '~써봤는데' 등 자연스러운 구어체.\n"
                    "'이것만 사세요' 강요 금지. 대신 '저는 이게 더 나은 것 같더라고요' 형식으로 자연스럽게 유도.\n"
                    "제목은 상품명 없이, 독자가 처한 상황·고민을 검색할 법한 키워드로 작성. (extra_context에 제목 지시가 있으면 그것을 우선 따를 것)\n"
                )
