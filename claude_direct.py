...
def _build_prompt(blog_id: str, keyword: str, extra_context: str = None) -> str:
    ...
    # 네이버 블로그 전용 문체 보정: 전형적인 AI 문장 패턴 억제
    if _is_naver:
        prompt += (
            "\n\n[문체 자연화 강제 규칙]\n"
            "- 문장을 교과서처럼 균일하게 쓰지 말고, 짧은 문장/중간 문장/긴 문장을 섞어라.\n"
            "- 같은 어미를 2문장 이상 연속 반복하지 마라(예: '~입니다' 3연속 금지).\n"
            "- '먼저/다음/마지막으로' 기계식 나열을 연속 사용하지 마라.\n"
            "- 각 H2 섹션에는 실제 상황 한 줄(실수/주의/체감 포인트)을 최소 1개 넣어라.\n"
            "- 아래 표현은 사용 금지: '알아보겠습니다', '정리해드릴게요', '도움이 되셨길 바랍니다', '지금 바로'.\n"
            "- 공공/정책 글은 단정적 과장 없이 사실 기반으로 쓰되, 문장 리듬은 딱딱하지 않게 유지하라.\n"
        )
...
3. 웹 블로그 문체(티스토리/워드프레스/블로그스팟)로 다듬어라.
   - 지나치게 공문서 같은 '~습니다' 일변도 금지
   - '~해요/~했어요'와 '~합니다'를 문맥에 맞게 섞되, 한 단락 안에서 어미를 과도 반복하지 마라.

4. 나열형("먼저~, 다음으로~, 마지막으로~") 대신 각 내용을 이유·효과까지 풀어서 써라.
   소제목 시작 문장을 모두 같은 패턴으로 쓰지 말고, 섹션마다 시작 방식을 바꿔라.

5. 마무리 문장에 "도움이 되셨으면", "유익한 정보" 류 금지. 자연스럽게 끝내라.

6. 문단 구분: 2~3문장마다 빈 줄 하나. 문장이 줄줄이 이어지는 덩어리 문단은 반드시 나눠라.

7. 모바일 가독성은 유지하되, 본문 한 줄을 기계적으로 잘게 분절하지 마라.
   워드프레스 가독성 기준으로 한 문단 2~4문장 흐름을 유지하고, 문장 내부 줄바꿈은 꼭 필요한 경우에만 사용한다.

8. 아래 전형적 AI 문구를 발견하면 자연어로 치환하라:
   - "핵심입니다", "완벽하게", "한눈에", "정리해보겠습니다", "도움이 되셨으면"
   - "지금 바로", "반드시", "무조건" 같은 과한 행동 유도 문구

9. 정부지원금/정책 주제는 숫자·기간·조건을 우선 배치하고,
   불확실한 표현("~로 알려져", "~예정")은 사실처럼 단정하지 마라.
