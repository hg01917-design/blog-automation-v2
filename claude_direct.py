...
def _build_prompt(blog_id: str, keyword: str, extra_context: str = None) -> str:
    ...
    if is_wordpress:
        ...
        _SECTION_FORMAT = (
            ...
            "본문 내용...\n"
            "[애드센스]       ← 해당 섹션 핵심 키워드 밀도 가장 높은 문단 끝에 삽입\n"
        ...
        _IMAGE_RULE = (
            "\n\n[이미지 규칙 — 필수]\n"
            "각 [H2]소제목[/H2] 바로 다음 줄에 [이미지N] 마커 삽입 (N=1부터 순서대로).\n"
            "본문 끝에 ===이미지=== 섹션:\n"
            "===이미지===\n[이미지1]\n- 프롬프트: (영어)\n- 파일명: (영문-소문자-하이픈.jpg)\n- alt: (한국어)\n===이미지끝===\n"
        )
    ...
    if instructions:
        ...
        prompt = (
            ...
            "소제목 3개 이상, 존댓말(해요체), 구체적 수치·날짜 포함.\n"
            f"{_SEO_TITLE_RULE}{_DEPTH_RULE}{_BANNED_EXPRESSIONS}{_SECTION_FORMAT}{_IMAGE_RULE}"
        )
