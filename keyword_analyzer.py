...
def generate_titles(keyword: str, blog_id: str, on_log=None) -> list[str]:
    ...
    prompt = (
        f"[출력 규칙] 제목 후보 5개만 출력. 번호·설명·마커(===) 없이 한 줄에 하나씩. 다른 텍스트 금지.\n\n"
        f"블로그 주제: {theme}\n"
        f"타겟 키워드: {keyword}\n\n"
        f"제목 조건:\n"
        f"- 길이: {title_len}\n"
        f"- 키워드를 제목 맨 앞에 정확히 포함 (변형 금지)\n"
        f"- 롱테일: 키워드 뒤에 구체적 수식어 붙이기 (신청방법/조건/나이/혜택금액/2026/비교 등)\n"
        f"- 후킹: 독자가 클릭하고 싶게 만드는 구체적 수치·기간·금액 포함\n"
        f"- 금지: 완벽정리/한눈에/꼼꼼히/총정리/모음 등 나열형 표현 절대 금지\n"
        f"- 금지: AI 클리셰(알아보겠습니다/살펴보겠습니다) 절대 금지\n"
        f"- 각 제목이 서로 다른 검색 의도를 커버할 것 (신청/조건/금액/나이/비교 등)\n\n"
        f"제목 5개 (번호 없이, 한 줄에 하나씩):"
    )
